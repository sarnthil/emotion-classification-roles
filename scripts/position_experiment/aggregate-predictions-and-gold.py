...
for dataset in Path("workdata/position-experiment/predictions").glob("*"):
    for file in dataset.glob("*.predictions"):
        part = file.stem
        gold_path = Path(f"workdata/position-experiment/datasets/{dataset.name}/{part}.jsonl.test")
        out_path = Path(f"workdata/position-experiment/predictions/{dataset.name}/{part}.aggregated")
